...
def get_model_context(
    ...
    if model_path.is_dir() and adapter_config_path.is_file():
        adapter_path = model_path
        adapter_config = json.loads(adapter_config_path.read_text())
        base_model_name_or_path = (
            adapter_config.get("base_model_name_or_path") or model_key
        )
        print(f"Detected PEFT adapter at {adapter_path}")
        print(f"Loading base model for adapter: {base_model_name_or_path}")

    # Transformers versions differ:
    # - newer: attn_implementation="flash_attention_2"
    # - older: use_flash_attention_2=True
    flash_kwarg_candidates: list[dict] = [{}]
    if use_flash_attention:
        flash_kwarg_candidates = [
            {"attn_implementation": "flash_attention_2"},
            {"use_flash_attention_2": True},
            {},
        ]

    dtype_kwarg_candidates: list[dict] = [
        {"dtype": dtype},
        {"torch_dtype": dtype},
    ]
    ...
    for flash_kwargs in flash_kwarg_candidates:
        for dtype_kwargs in dtype_kwarg_candidates:
            try:
                ...
                if use_flash_attention:
                    if flash_kwargs:
                        print(
                            "Loaded model with flash attention kwargs: "
                            f"{flash_kwargs}"
                        )
                    else:
                        print(
                            "Flash attention requested but not supported in this setup. "
                            "Falling back to default attention."
                        )
                break
            except TypeError as error:
                message = str(error)
                unsupported_flash_kwarg = (
                    "attn_implementation" in message
                    or "use_flash_attention_2" in message
                )
                unsupported_dtype_kwarg = (
                    "dtype" in dtype_kwargs and "unexpected keyword argument 'dtype'" in message
                )
                if unsupported_flash_kwarg and flash_kwargs:
                    last_error = error
                    break
                if unsupported_dtype_kwarg:
                    last_error = error
                    continue
                raise
                ...
                flash_attn_unavailable = (
                    "FlashAttention2" in message or "flash_attn" in message
                )
                if flash_attn_unavailable and flash_kwargs:
                    last_error = error
                    break
                raise
    ...
    if adapter_path is not None:
        try:
            ...
            raise ImportError(
                "Found adapter checkpoint but `peft` is not installed."
            ) from error
